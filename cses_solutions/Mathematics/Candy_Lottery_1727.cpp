...
vector<vector<int>>pascal_triangle(){
    ...
    for(int i=2;i<=100;i++){
        ...
        temp.emplace_back(1);
        for(int j=1;j<i;j++){
            temp.emplace_back(res[i-1][j]+res[i-1][j-1]);
        }
        temp.emplace_back(1);
...
int vis[101][101];
long double solve(int index,int mx,int k,int n){
    if(index==n){
        return mx;
    }
    if(vis[index][mx]==1)
    ...
    long double res=0;
    for(int i=1;i<=k;i++){
        res+=solve(index+1,max(i,mx),k,n)*1.0/k;
    }
...
signed main() {
    ...
    int n,k;
    cin>>n>>k;
    cout<<fixed<<showpoint<<setprecision(6)<<solve(0,0,k,n)<<line;
}