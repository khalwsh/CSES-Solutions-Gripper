...
int catalan(int n){
    ...
    res=0;
    for(int i=0;i<=n-1;i++){
        res=add(res,(catalan(i)* catalan(n-i-1)),mod);
    }
    return res;
}
int nCr(int a,int b){
    vector<int>fact= factorial(N,mod);
    return mul(fact[a],mul(fast_power(fact[b],mod-2,mod), fast_power(fact[a-b],mod-2,mod),mod),mod);
}
signed main() {
    khaled
    int n;
    cin>>n;
    cout<<((n&1)?0:mul(nCr(n,n/2), fast_power(n/2+1,mod-2,mod),mod))<<line;
}