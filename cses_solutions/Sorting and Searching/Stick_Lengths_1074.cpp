...
There are nnn sticks with some lengths. Your task is to modify the sticks so that each stick has the same length.
You can either lengthen and shorten each stick. Both operations cost xxx where xxx is the difference between the new and original length.
What is the minimum total cost?
Input
The first input line contains an integer nnn: the number of sticks.
Then there are nnn integers: p1, p2, …, pnp_1, p_2, \ldots, p_np1​, p2​, …, pn​: the lengths of the sticks.
Output
...
/*--------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------*/
int solve(vector<int>&v,int avg){
    int res=0;
    for(auto &val:v){
        res+=abs(val-avg);
    }
...
signed main() {
    ...
    //cout<<sum/n<<" "<<(sum+n-1)/n<<line;
    cout<<min(solve(v,v[n/2]),solve(v,v[(n+1)/2]))<<line;
}